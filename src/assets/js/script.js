...
const animateElements = document.querySelectorAll('.timeline-item, .skill-category, .stat-item, .education-card, .about-text');
...
if (contactForm) {
    contactForm.addEventListener('submit', (e) => {
        ...
        // Create mailto link
        const mailtoLink = `mailto:l5r@outlook.sa?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(
            `Name: ${name}\nEmail: ${email}\n\nMessage:\n${message}`
        )}`;
...
downloadButtons.forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        if (typeof gtag !== 'undefined') {
            gtag('event', 'download', {
                'event_category': 'CV',
                'event_label': 'Ahmed_Alzahrani_CV.pdf'
            });
        }
    });
});
...
// Optional: Allow users to change theme color
const colors = {
    purple: { primary: '#6366f1', secondary: '#0ea5e9' },
...
function changeThemeColor(colorName) {
    if (colors[colorName]) {
        document.documentElement.style.setProperty('--primary-color', colors[colorName].primary);
        document.documentElement.style.setProperty('--secondary-color', colors[colorName].secondary);
...
console.log(
    '%cWelcome to Ahmed Alzahrani\'s Portfolio!',
...
window.addEventListener('load', () => {
    ...
    setTimeout(() => {
        // Uncomment if you want a welcome message
